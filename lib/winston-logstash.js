...
var ENOTFOUND_REGEXP = /ENOTFOUND/;
...
var Logstash = exports.Logstash = function (options) {
  ...
  this.silent_fail         = options.silentFail || false;
...
Logstash.prototype.connect = function () {
  ...
  this.socket.on('error', function (err) {
    ...
    if (!ECONNREFUSED_REGEXP.test(err.message) && !ENOTFOUND_REGEXP.test(err.message)) {
      tryReconnect = false;
      self.emit('error', err);
    }
  });
  ...
  this.socket.on('close', function (had_error) {
    ...
    if (self.max_connect_retries < 0 || self.retries < self.max_connect_retries) {
      ...
      self.silent = true;
      if (self.silent_fail) {
        console.log('Max retries reached, the logstash server could not be reached, logstash will not be used');
      } else {
